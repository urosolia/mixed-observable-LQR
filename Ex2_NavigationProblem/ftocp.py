...
class FTOCP(object):
	...
	def build(self):
		...
		self.regVar = [Variable((self.numReg, self.Nb), boolean=True) for a in range(0, self.numSegments)] # Initialize vector of variables
		...
		for j in range(0, self.numSegments):
			...
			for i in range(0, self.Nb):
				constr += [self.x[j][:,i+1] == self.A @ self.x[j][:,i] + self.B @ self.u[j][:,i],
							...
							self.F@self.x[j][0:2,i+1] <= self.bMatrix@self.regVar[j][:, i],
							sum(self.regVar[j][:, i]) == 1]							
	...
	def solve(self, x0, b0):
		...
		self.bigM.value = np.max(self.D_reg)**(self.P-1) * np.max(1/b0[0]) + 1.0
		print("self.bigM.value: ", self.bigM.value)
		self.x0.value   = x0
		# print(1/b0[0])
