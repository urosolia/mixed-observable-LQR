...
class polytope(object):
	...
	def __init__(self, F=None, b=None, vertices=None, printLevel = 0):
	...
	def computeVRep(self):
		...
		if(self.printLevel>=1):print("vertices: ", self.vertices)
	...
	def plot2DPolytopePath(self, color, ax, label = None):
		vertices  = pypoman.polygon.compute_polygon_hull(self.F, self.b)
		ax.add_patch(patches.Polygon(vertices, alpha=.1, edgecolor=color,facecolor=color,label=label))

	def plot2DPolytopeObstacles(self, color, ax, label = None):
		vertices  = pypoman.polygon.compute_polygon_hull(self.F, self.b)
		ax.add_patch(patches.Polygon(vertices, alpha=.5, edgecolor=color,facecolor=color,label=label, hatch='//'))
